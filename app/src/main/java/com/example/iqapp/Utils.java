...
public final class Utils {
    ...
    private static List<Blog> extractFeatureFromJson(String blogJSON){
        ...
        try {
            ...
            JSONArray blogArray = baseJsonResponse.getJSONArray("items");
            for (int i=0; i < blogArray.length();i++){
                JSONObject currentBlog = blogArray.getJSONObject(i);
                String title = currentBlog.getString("title");
                String thumbnail = currentBlog.getString("thumbnail");
                String link = currentBlog.getString("guid");
                String desc = currentBlog.getString("description");
                Blog blog = new Blog(title,thumbnail,link,desc);
                blogs.add(blog);
            }
        }catch (JSONException e) {
            Log.e("QueryUtils", "Problem parsing the blog JSON results", e);
        }
        Log.v(LOG_TAG,"applied");
        return  blogs;
    }
}
